classdef manip < RobotBaseClass
    %% LinearUR10 UR10 on a non-standard linear rail created by a student

    properties(Access = public)              
        plyFileNameStem = 'manip';
    end
    
    methods
%% Constructor
        function self = manip(baseTr)
			self.CreateModel();
            if nargin < 1			
				baseTr = eye(4);				
            end
...
%% CreateModel
        function CreateModel(self)   
            ...
            %link(2) = Link([0     0.1697      0        -pi/2   1]);
            link(3) = Link([0     0.176       1.9129   -pi     0]);
            link(4) = Link([0     0.12781     1.9716	pi      0]);
